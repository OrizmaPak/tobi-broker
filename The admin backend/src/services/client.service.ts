...
export async function clientDashboard(clientId: string) {
  ...
  return {
    ...
    metrics: {
      ...
      profileCompletion: kycStatus === "APPROVED" ? 100 : client.emailVerifiedAt ? 65 : 35,
...
export async function clientSnapshot(clientId: string) {
  ...
  const [products, instruments, optionContracts, globalReports, capabilities] = await Promise.all([
