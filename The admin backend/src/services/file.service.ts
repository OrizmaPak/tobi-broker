...
export async function uploadPublicImageToCloudinary(input: {
  folder: string;
  fileName: string;
  mimeType: string;
  base64: string;
  ...
  if (!result.secure_url || !result.public_id) throw new ApiError(502, "Cloudinary upload failed", "STORAGE_UPLOAD_FAILED");
  return {
    storageKey: `cloudinary:image:public:${result.public_id}`,
...
export async function storePublicImage(input: {
  ...
  const stored = hasCloudinaryConfig()
    ? await uploadPublicImageToCloudinary({ folder: `${input.ownerType.toLowerCase()}/${input.ownerId}/${input.category}`, fileName: input.fileName, mimeType: input.mimeType, base64: input.base64 })
    : await storePublicVercelBlob(pathname, body, input.mimeType, input.fileName);
  return prisma.storedFile.create({
    data: {
      ownerType: input.ownerType,
      ownerId: input.ownerId,
...
async function storePublicVercelBlob(pathname: string, body: Buffer, mimeType: string, fileName: string) {
