(function () {
  ...
  const navGroups = [
    ...
    ["Investments", [["Markets", "markets.html", "trend"], ["Instruments", "instruments.html", "chart"], ["Portfolio Products", "portfolio-products.html", "briefcase"], ["Client Investments", "client-investments.html", "chart"], ["Dividends & Profits", "dividends-profits.html", "coins"], ["Trading Orders", "orders.html", "list"], ["Options Access", "options.html", "shield"], ["Risk & Compliance", "risk.html", "alert"]]],
  ...
  function productBannerHtml(product, mode) {
    ...
    const className = mode === "hero"
      ? "product-banner product-banner-hero"
      : mode === "client"
        ? "product-banner product-banner-client"
        : mode === "mobile"
    ...
    return '<div class="' + className + '">' + media + '<div class="product-banner-glow"></div><div class="product-banner-overlay"><em>Managed portfolio</em><strong>' + escapeHtml(name) + '</strong><small>' + escapeHtml(productReturnText(product)) + '</small><span class="product-banner-assets">' + escapeHtml(productInstrumentSummary(product)) + '</span></div></div>';
  }

  function productBannerPreviewHtml(product) {
    return '<div class="product-banner-live-preview"><div class="product-banner-preview-head"><div><strong>Live client preview</strong><span>This is how the uploaded background and code-rendered content will sit together.</span></div><span>1600 x 900 safe composition</span></div><div class="product-banner-preview-grid"><div><p>Desktop card</p>' + productBannerHtml(product, "client") + '</div><div><p>Mobile crop</p>' + productBannerHtml(product, "mobile") + '</div></div></div>';
  }
  ...
  function productBannerPrompt(product) {
    ...
    return [
      "Create a background-only premium investment product banner image at exactly 1600 x 900 pixels, 16:9 aspect ratio.",
      "Absolute rule: do not place any text in the image. No words, numbers, percentages, tickers, chart labels, signatures, captions, logos, watermarks, or text-like symbols.",
      "BullPort will add the product name, return percentage, and instrument summary later with code, so the generated image must only be pictorial background art.",
      "Use this product context only to choose the visual theme; do not write it into the image: product name \"" + (product?.name || "Portfolio product") + "\", advertised return \"" + productReturnText(product) + "\", instruments \"" + productInstrumentSummary(product) + "\".",
      "Product strategy for visual mood only: " + (product?.description || "Broker-managed diversified investment portfolio.") + ".",
      "Risk profile for visual mood only: " + (product?.risk || product?.riskLevel || "Moderate") + ".",
      allocations ? "Use subtle pictorial motifs inspired by these instruments and weights, without labels: " + allocations + "." : "Use subtle pictorial motifs for diversified markets, instruments, commodities, charts, and global allocation, without labels.",
      "Composition requirement: reserve the lower-left and center-left 50% of the image as a dark, clean, low-detail safe zone for code-rendered text. No important objects, bright highlights, faces, icons, or busy patterns in that safe zone.",
      "Place the main pictorial content on the right side and upper-right background: abstract market forms, instrument/commodity visuals, dimensional chart shapes without labels, global finance atmosphere, premium depth.",
      "Visual direction: modern institutional wealth management, cinematic lighting, confident, premium, intelligent, captivating, not generic.",
      "Palette: deep navy base, emerald growth accents, warm gold highlights, with smooth contrast that keeps the safe text area readable.",
      "Final output must be a clean image background that makes the later code overlay fit naturally and look intentional."
    ].join("\n");
  }
  ...
  async function loadBackendData() {
    ...
    const isDividendsProfitPage = currentFile() === "dividends-profits.html";
    ...
    const [clients, kyc, kycRequirements, deposits, withdrawals, beneficiaries, products, investments, payouts, profitSchedules, markets, instruments, tickets, auditLogs, approvals, orders, positions, optionsApplications, riskAlerts, reports, notifications, adminUsers, settingsRows, tasks] = await Promise.all([
      ...
      isDividendsProfitPage ? tryApiResponse(profitSchedulesPath) : tryApi(profitSchedulesPath),
    ...
    const profitScheduleRows = currentFile() === "dividends-profits.html" && profitSchedules && !Array.isArray(profitSchedules) ? profitSchedules.data : profitSchedules;
    appState.profitScheduleMeta = currentFile() === "dividends-profits.html" && profitSchedules && !Array.isArray(profitSchedules) && profitSchedules.meta ? profitSchedules.meta : { page: 1, limit: 20, total: Array.isArray(profitScheduleRows) ? profitScheduleRows.length : 0, pages: 1 };
    if (Array.isArray(profitScheduleRows)) {
      data.profitSchedules = profitScheduleRows.map((row) => {
        ...
        const mode = row.receipt?.mode || row.mode || "WALLET";
        const posted = row.receipt ? '<span class="muted">' + escapeHtml(row.receipt.reference) + '</span>' : '<span class="muted">No receipt</span>';
        ...
        return [
          ...
          payoutToneCell(row.type || "BOT_PROFIT", amount),
  ...
  function payoutToneCell(value, amountValue) {
    ...
    const text = label(value || "Profit");
    const isLoss = amount < 0 || /loss/i.test(text);
    return '<span class="badge ' + (isLoss ? "danger" : "success") + '">' + escapeHtml(text) + "</span>";
  }

  function pnlAmountCell(value) {
    const amount = Number(value || 0);
    const className = amount < 0 ? "danger" : amount > 0 ? "success" : "info";
    const prefix = amount > 0 ? "+" : "";
    return '<span class="badge ' + className + '">' + prefix + formatMoney(amount) + "</span>";
  }
  ...
  function instrumentsPage() {
    ...
    const searchBar = '<form class="filter-bar" data-instrument-search-form><label class="filter-search">' + svg("grid") + '<input data-instrument-backend-search value="' + escapeHtml(searchQuery) + '" placeholder="Search symbol, instrument, market, category..." /></label><button class="btn primary" type="submit">Search</button>' + (searchQuery ? '<a class="btn" href="' + clearSearchHref() + '">Clear</a>' : "") + '</form>';
  ...
  function productDetailPage() {
    ...
    const bannerManager = section("Product banner", "Upload the marketing banner clients will see on this product, or generate a background-only AI image prompt with the required 1600 x 900 dimension.", '<div class="product-banner-direct"><input type="hidden" name="productName" value="' + escapeHtml(p.name) + '"><input type="hidden" name="productReturnMin" value="' + escapeHtml(p.projectedReturnMin) + '"><input type="hidden" name="productReturnMax" value="' + escapeHtml(p.projectedReturnMax) + '"><div class="product-banner-editor-preview" data-product-banner-preview>' + productBannerPreviewHtml(p) + '</div><div class="product-banner-controls"><label class="product-banner-field">Upload banner image <span class="field-optional">JPG, PNG, WebP</span><input name="productBannerFile" data-product-banner-file type="file" accept="image/jpeg,image/png,image/webp"></label><label class="product-banner-field">Banner URL <span class="field-optional">Saved to product</span><input name="productBannerUrl" data-product-banner-url maxlength="1000" value="' + escapeHtml(p.bannerUrl || "") + '" placeholder="https://.../portfolio-banner.jpg"></label></div><div class="product-banner-prompt-panel"><div class="product-banner-prompt-head"><div><strong>AI background prompt</strong><span>Predefined size: 1600 x 900px. No text inside the image; pictorial content is positioned around the code overlay safe zone.</span></div><div class="action-row"><button class="btn" type="button" data-action="product-banner-prompt">Generate prompt</button><button class="btn" type="button" data-action="product-banner-prompt-copy">Copy prompt</button></div></div><textarea data-product-banner-prompt readonly placeholder="Click Generate prompt to create a ready-to-use background-only banner prompt for this product."></textarea></div></div>', '<button class="btn primary" type="button" data-action="product-banner-save">Save banner</button>');
  ...
  function withdrawalSettingsPanel() {
    ...
    return section("Payout settings", "Define bank and crypto payout destination forms shown in the client portal before a withdrawal destination can be reviewed.", '<div class="deposit-method-toolbar"><div><strong>' + activeCount + ' active route' + (activeCount === 1 ? "" : "s") + '</strong><span>Each route can collect its own required verification fields before admin approval.</span></div><div class="action-row"><button class="btn primary" type="button" data-action="withdrawal-method-new" data-method-type="BANK">Add bank payout form</button><button class="btn" type="button" data-action="withdrawal-method-new" data-method-type="CRYPTO">Add crypto payout form</button><button class="btn" type="button" data-action="setting-edit" data-setting-key="withdrawal.methods">Edit JSON</button></div></div>' + table(["Type", "Method", "Currency", "Fields", "Status", "Action"], rows));
  }
  ...
  function bodyFor(file) {
    switch (file) {
      ...
      case "dividends-profits.html": return dividendsProfitsPage();
  ...
  async function submitInstrument(id) {
    ...
    const payload = {
      ...
      logoUrl: root?.querySelector('[name="instrumentLogoUrl"]')?.value.trim() || undefined,
  ...
  async function saveWithdrawalMethods(value) {
    const description = "Accepted client payout routes and beneficiary verification field requirements.";
