(function () {
  ...
  function instrumentsPage() {
    ...
    return section("Market filter", "View instruments by their assigned market.", '<div class="action-row">' + marketTabs + "</div>") +
      section("Instrument universe", "Control visibility, tradability, investability, and restrictions across supported asset classes.", filterableTable("Search symbol, market, category...", ["Symbol", "Instrument", "Category", "Market", "Risk", "Status", "Action"], shownInstruments.map((row) => [row.symbol, '<span class="instrument-logo-cell">' + (row.logoUrl ? '<span class="instrument-logo-frame"><img src="' + escapeHtml(row.logoUrl) + '" alt="" loading="lazy"></span>' : '<span class="instrument-logo-placeholder">' + escapeHtml(row.symbol.slice(0, 1).toUpperCase()) + '</span>') + '<strong>' + escapeHtml(row.name) + '</strong></span>', row.category, row.marketRecord?.name || row.market, badge(label(row.riskLevel)), badge(label(row.status)), '<button class="btn" type="button" data-action="instrument-edit" data-instrument-id="' + escapeHtml(row.id) + '">Edit</button>'])), action);
  }
